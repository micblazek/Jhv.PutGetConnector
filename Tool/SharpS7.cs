...
namespace Jhv.PutGetConnector.Tool
{
    ...
    public static class S7
    {
        ...
        {
            //Michal edit 4.12.2017
            int tmp = NewMethod(Buffer, Pos);
            if (tmp <= 32767)
                return tmp;
            else
                return tmp - 65536;
        ...
        private static int NewMethod(byte[] Buffer, int Pos)
        {
    ...
    public class S7Client
    {
        ...
        internal int ReadArea(byte s7AreaDB, int dbTypList, int v1, int v2, int s7WLByte, byte[] dbBuffer853_p1, ref object size)
        {
